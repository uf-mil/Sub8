...
void GazeboRosImageSonar::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  this->newDepthFrameConnection = this->depthCamera->ConnectNewDepthFrame(
      std::bind(&GazeboRosImageSonar::OnNewDepthFrame, this, std::placeholders::_1, std::placeholders::_2,
                std::placeholders::_3, std::placeholders::_4, std::placeholders::_5));

  this->newRGBPointCloudConnection = this->depthCamera->ConnectNewRGBPointCloud(
      std::bind(&GazeboRosImageSonar::OnNewRGBPointCloud, this, std::placeholders::_1, std::placeholders::_2,
                std::placeholders::_3, std::placeholders::_4, std::placeholders::_5));

  this->newImageFrameConnection = this->depthCamera->ConnectNewImageFrame(
      std::bind(&GazeboRosImageSonar::OnNewImageFrame, this, std::placeholders::_1, std::placeholders::_2,
                std::placeholders::_3, std::placeholders::_4, std::placeholders::_5));
...
void GazeboRosImageSonar::Advertise()
{
  ros::AdvertiseOptions point_cloud_ao = ros::AdvertiseOptions::create<sensor_msgs::PointCloud2>(
      this->point_cloud_topic_name_, 1, boost::bind(&GazeboRosImageSonar::PointCloudConnect, this),
      boost::bind(&GazeboRosImageSonar::PointCloudDisconnect, this), ros::VoidPtr(), &this->camera_queue_);
  ...
  ros::AdvertiseOptions depth_image_ao = ros::AdvertiseOptions::create<sensor_msgs::Image>(
      this->depth_image_topic_name_, 1, boost::bind(&GazeboRosImageSonar::DepthImageConnect, this),
      boost::bind(&GazeboRosImageSonar::DepthImageDisconnect, this), ros::VoidPtr(), &this->camera_queue_);
  this->depth_image_pub_ = this->rosnode_->advertise(depth_image_ao);

  ros::AdvertiseOptions normal_image_ao = ros::AdvertiseOptions::create<sensor_msgs::Image>(
      this->depth_image_topic_name_ + "_normals", 1, boost::bind(&GazeboRosImageSonar::NormalImageConnect, this),
      boost::bind(&GazeboRosImageSonar::NormalImageDisconnect, this), ros::VoidPtr(), &this->camera_queue_);
  this->normal_image_pub_ = this->rosnode_->advertise(normal_image_ao);

  ros::AdvertiseOptions multibeam_image_ao = ros::AdvertiseOptions::create<sensor_msgs::Image>(
      this->depth_image_topic_name_ + "_multibeam", 1, boost::bind(&GazeboRosImageSonar::MultibeamImageConnect, this),
      boost::bind(&GazeboRosImageSonar::MultibeamImageDisconnect, this), ros::VoidPtr(), &this->camera_queue_);
  this->multibeam_image_pub_ = this->rosnode_->advertise(multibeam_image_ao);

  ros::AdvertiseOptions sonar_image_ao = ros::AdvertiseOptions::create<sensor_msgs::Image>(
      this->depth_image_topic_name_ + "_sonar", 1, boost::bind(&GazeboRosImageSonar::SonarImageConnect, this),
      boost::bind(&GazeboRosImageSonar::SonarImageDisconnect, this), ros::VoidPtr(), &this->camera_queue_);
  this->sonar_image_pub_ = this->rosnode_->advertise(sonar_image_ao);

  ros::AdvertiseOptions raw_sonar_image_ao = ros::AdvertiseOptions::create<sensor_msgs::Image>(
      this->depth_image_topic_name_ + "_raw_sonar", 1, boost::bind(&GazeboRosImageSonar::RawSonarImageConnect, this),
      boost::bind(&GazeboRosImageSonar::RawSonarImageDisconnect, this), ros::VoidPtr(), &this->camera_queue_);
  ...
  ros::AdvertiseOptions depth_image_camera_info_ao = ros::AdvertiseOptions::create<sensor_msgs::CameraInfo>(
      this->depth_image_camera_info_topic_name_, 1, boost::bind(&GazeboRosImageSonar::DepthInfoConnect, this),
      boost::bind(&GazeboRosImageSonar::DepthInfoDisconnect, this), ros::VoidPtr(), &this->camera_queue_);
  this->depth_image_camera_info_pub_ = this->rosnode_->advertise(depth_image_camera_info_ao);

 /*ros::AdvertiseOptions ping_msg_ao = ros::AdvertiseOptions::create<sub8_gazebo::simulated_sonar_ping>(
    sub8_gazebo::simulated_sonar_ping, 1, boost::bind(&GazeboRosImageSonar::SonarPingConnect, this),
    boost::bind(&GazeboRosImageSonar::SonarPingDisconnect, this), ros::VoidPtr());
  this->sonar_ping_msg_pub_ = this->rosnode_->advertise(ping_msg_ao);
  */
  this->sonar_ping_msg_pub_ = this->rosnode_->advertise<sub8_gazebo::simulated_sonar_ping>("simRanges", 5);
  this->ping_ptr.reset(new sub8_gazebo::simulated_sonar_ping);
...
void GazeboRosImageSonar::SonarPingConnect()
{
  this->sonar_ping_count_++;
}

void GazeboRosImageSonar::SonarPingDisconnect()
{
  this->sonar_ping_count_--;
}
...
void GazeboRosImageSonar::OnNewDepthFrame(const float* _image, unsigned int _width, unsigned int _height,
...
///////////////////////////////////////////////////////////////////////////////
// Update the controller
void GazeboRosImageSonar::OnNewRGBPointCloud(const float* _pcd, unsigned int _width, unsigned int _height,
                                             unsigned int _depth, const std::string& _format)
{
  if (!this->initialized_ || this->height_ <= 0 || this->width_ <= 0)
    return;
...
////////////////////////////////////////////////////////////////////////////////
// Update the controller
void GazeboRosImageSonar::OnNewImageFrame(const unsigned char* _image, unsigned int _width, unsigned int _height,
                                          unsigned int _depth, const std::string& _format)
{
  if (!this->initialized_ || this->height_ <= 0 || this->width_ <= 0)
    return;

  // ROS_ERROR_NAMED("depth_camera", "camera_ new frame %s
  // %s",this->parentSensor_->GetName().c_str(),this->frame_name_.c_str());
...
bool GazeboRosImageSonar::FillPointCloudHelper(sensor_msgs::PointCloud2& point_cloud_msg, uint32_t rows_arg,
...
// Fill depth information
bool GazeboRosImageSonar::FillDepthImageHelper(sensor_msgs::Image& image_msg, uint32_t rows_arg, uint32_t cols_arg,
...
cv::Mat GazeboRosImageSonar::ConstructSonarImage(cv::Mat& depth, cv::Mat& normals)
{
  ...
  if (dist_matrix_.empty())
  {
    ...
    for (int i = 0; i < depth.cols; i++)
      t_x.push_back((float(i) - this->cx_) / this->focal_length_);
    for (int i = 0; i < depth.rows; i++)
      t_y.push_back((float(i) - this->cy_) / this->focal_length_);
    cv::Mat X, Y;
...
// Constructs the raw sonar image
cv::Mat GazeboRosImageSonar::ConstructScanImage(cv::Mat& depth, cv::Mat& SNR)
{
  int rows = 400;    // TODO: add a parameter for this
  float range = 8.;  // TODO: get this from the sensor config instead

  float fov = depthCamera->HFOV().Degree();
  int cols = 2 * int(float(rows) * sin(M_PI / 180. * fov / 2.)) + 20;  //~36.4 for 130FOV

  // M gets crops image based on FOV, should at least
  ...
  int ping_count = 0;
  cv::Point scan_origin = cv::Point(double(depth.cols / 2.0), 0.0);
  ...
  for (int i = 0; i < depth.rows; ++i)
  {
    for (int j = 0; j < depth.cols; ++j)
    {
      ...
      x *= z;
      y *= z;
      x_trans = x - (scan.cols/2);
      y_trans = scan.rows - y;

      cv::Point hit = cv::Point(double(x), double(y));
      ...
      if (pi < scan.rows && pi > 0 && pj < scan.cols && pj > 0 && x * x + z * z < range * range)
      {
        ...
        //this->ping_ptr->header.frame_id = this->frame_name.nsec
        //ROS_INFO("depth val: %f\n",d);
        ranges.push_back(d);
        intensities.push_back((uint16_t)a);
        //bearings.push_back((float)(this->CalculateBearing(hit, scan_origin)));
        bearings.push_back(atan2(y_trans - 0, x_trans - 0) * 180 / M_PI);
        //this->ping_ptr->intensities[ping_count] = (uint16_t)a;
        //this->ping_ptr->ranges[ping_count] = d;
        //this->ping_ptr->bearings[ping_count] = float(this->CalculateBearing(hit, scan_origin));
        ping_count++;
 ...
 // this->ping_ptr->header.frame_id = "ranges";
  this->ping_ptr->header.stamp.sec = this->depth_sensor_update_time_.sec;
  this->ping_ptr->header.stamp.nsec = this->depth_sensor_update_time_.nsec;
  ranges.resize(ping_count);
  intensities.resize(ping_count);
  bearings.resize(ping_count);
  this->ping_ptr->ranges = ranges;
  this->ping_ptr->intensities = intensities;
  this->ping_ptr->bearings = bearings;
  this->sonar_ping_msg_pub_.publish(this->ping_ptr);
  this->ApplyMedianFilter(scan);
  this->ApplySpeckleNoise(scan, fov);
  // this->ApplySmoothing(scan, fov);

  //ToDo: send over to constructPingMsg
...
double GazeboRosImageSonar::CalculateBearing(const cv::Point& v1, const cv::Point& v2)
{
...
cv::Mat GazeboRosImageSonar::ConstructPingMsg()
{
...
pub_disparity_ = comm_nh.advertise<stereo_msgs::DisparityImage > ("depth/disparity", 5, subscriberChanged2,
subscriberChanged2);
void GazeboRosImageSonar::PublishDisparityImage(const DepthImage& depth, ros::Time time)
{
  ...
  depth.fillDisparityImage (depth_width_, depth_height_, reinterpret_cast<float*>(&disp_msg->image.data[0]),
disp_msg->image.step);
