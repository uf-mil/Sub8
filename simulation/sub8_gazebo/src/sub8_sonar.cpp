...
void GazeboRosImageSonar::Advertise()
{
	 ...
	 */
	this->sonar_ping_msg_pub_ = this->rosnode_->advertise<mil_blueview_driver::BlueViewPing>("blueview_driver/ranges", 5);
//	this->ping_ptr.reset(new sub8_gazebo::simulated_sonar_ping);
	this->ping_ptr.reset(new mil_blueview_driver::BlueViewPing);
}
...
/////////////////////////////////////////////////////////////////////////////
// Speckles make it look more realistic
...
{
	int rows = 860;     // TODO: add a parameter for this
	...
	for (int i = 0; i < depth.rows; ++i)
	{
		for (int j = 0; j < depth.cols; ++j)
		{
			...
			{
				x_trans = pj - (scan.cols / 2);
				y_trans = scan.rows - pi;
				scan.at<float>(pi, pj) = a;  

				// ROS_INFO("depth val: %f\n",d);

				// Scales intensity so farther away objects are visible
				int scalar;
				ranges.push_back((float)d);
				if (d < 4.0)
					scalar = 1000;
				if (4.0 <= d < 8.0)
					scalar = 1300;
				...
				intensities.push_back(uint16_t(a * scalar));
				angle = atan2(y_trans - 0.0, -x_trans - 0.0);

				if (0.0 < angle < M_PI / 2.0)
					bearings.push_back(float((M_PI / 2.0) - angle));
				else if (M_PI / 2.0 < angle < M_PI)
					bearings.push_back(float(M_PI - angle));
				// else if(angle > M_PI/2.0)
				// bearings.push_back(float(-(angle -(M_PI/2.0))));
				else
	...
	this->ping_ptr->header.frame_id = "blueview/sonar_link";
...
cv::Mat GazeboRosImageSonar::ConstructVisualScanImage(cv::Mat& raw_scan)
{
	...
	// scan.setTo(blue);
	scan.setTo(darkGrey);
	// what the FUCKKCKCk'cFBGFSTHDFDsETJKN<BHMGDJwsavcxznb<mknbvftegxh bjefdsm

	cv::Point center(scan.cols / 2, scan.rows);
	cv::Size axes(scan.rows + 3, scan.rows + 3);
	cv::ellipse(scan, center, axes, -90, -fov / 2., fov / 2., black, -1);  
// maps the color. TODO: scale values and use cv::ApplyColorMap
	for (int i = 0; i < scan.rows; ++i)
	{
		for (int j = 0; j < scan.cols; ++j)
		{
