...
class goal_proxy(object):
    ...
    '''
        ...
        ability to specifif location information
    '''
    ...
    def send_goal(self, goal_pos, timeout = 10):
        """ Send lamove_result move to the server to complete """
        ...
        if finished_in_time:
            ...
            if self.testing: return goal_pos
            return True
        else:
    ...
    def absolute_move(self, x = None, y = None, z = None, qx = None, qy = None, qz = None, qw = None, timeout = 10):
        ...
        if x == None: x = self.current_pos.pose.pose.x 
        if y == None: y = self.current_pos.pose.pose.y,
        if z == None: z = self.current_pos.pose.pose.z, 
        if qx == None: qx = self.current_pos.pose.orientation.x,
        if qy == None: qy = self.current_pos.pose.orientation.y, 
        if qz == None: qz = self.current_pos.pose.orientation.z,
        if qw == None: qw = self.current_pos.pose.orientation.w,
    ...
    def relative_move(self, x = 0, y = 0, z = 0, tx = 0, ty = 0, tz = 0, timeout = 10):
        ...
        to_send.pose.pose.position.x = self.current_pos.pose.pose.position.x + self.relative_goal.x
        to_send.pose.pose.position.y = self.current_pos.pose.pose.position.y + self.relative_goal.y
        to_send.pose.pose.position.z = self.current_pos.pose.pose.position.z + self.relative_goal.z

        # This section adds the desired axis rotation to current position
        # convert from quaternion to numpy array 
        cur_rotation = gh.quat_to_rotvec(gh.quat_to_np(self.current_pos.pose.pose.orientation))
        ...
        quat = gh.rotvec_to_quat(new_rotation)
        # set quaternion to send as the new quaternion
        to_send.pose.pose.orientation = gh.np_to_quat(quat)
        # send goal to server to complete within timeout
        move_result = self.send_goal(to_send, timeout)
        return move_result
