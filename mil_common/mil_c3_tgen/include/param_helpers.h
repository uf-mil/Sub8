...
bool _getParam(ros::NodeHandle &nh, const std::string &name,
               Eigen::Matrix<T, N, 1> &res) {
  XmlRpc::XmlRpcValue my_list;
  if (!nh.getParam(name, my_list))
    return false;
  require(my_list.getType() == XmlRpc::XmlRpcValue::TypeArray, "param ", name,
          " must be an array");
  if (N != Eigen::Dynamic) {
    require(my_list.size() == N, "param ", name, "must have length ", N,
            " (is ", my_list.size(), ")");
  }
...
bool _getParam(ros::NodeHandle &nh, const std::string &name,
  ...
  require(my_list[0].getType() == XmlRpc::XmlRpcValue::TypeArray, "param ",
          name, "[0] must be a list");
  if (M != Eigen::Dynamic) {
    require(my_list[0].size() == M, "param ", name, "[0] must have length ", M,
            " (is ", my_list[0].size(), ")");
  }
  ...
  for (int32_t i = 0; i < my_list.size(); i++) {
    ...
    require(row.getType() == XmlRpc::XmlRpcValue::TypeArray, "param ", name,
            "[", i, "] must be a list");
    require(row.size() == my_list[0].size(), "param ", name, "[", i,
            "]'s length doesn't match");
...
template <typename T> T getParam(ros::NodeHandle &nh, std::string name) {
  T res;
