#ifndef MSG_HELPERS_H
#define MSG_HELPERS_H

#include <Eigen/Dense>
#include <tf/tf.h>

template <class T> inline T make_rgba(double r, double g, double b, double a) {
  T c;
  c.r = r;
  c.g = g;
  c.b = b;
...
template <class T> inline T make_xyz(double x, double y, double z) {
  T p;
  p.x = x;
  p.y = y;
  p.z = z;
...
template <class T> inline T vec2xyz(Eigen::Vector3d v) {
  return make_xyz<T>(v(0), v(1), v(2));
}
template <class T> inline T vec2xyz(tf::Vector3 v) {
  return make_xyz<T>(v.x(), v.y(), v.z());
}
template <class T> inline Eigen::Vector3d xyz2vec(T m) {
...
template <class T> inline T make_xyzw(double x, double y, double z, double w) {
  T q;
  q.x = x;
  q.y = y;
  q.z = z;
...
template <class T> inline T vec2xyzw(Eigen::Vector4d v) {
  return vec2xyzw<T>(v(0), v(1), v(2), v(3));
}
template <class T> inline Eigen::Quaterniond xyzw2quat(T m) {
  return Eigen::Quaterniond(m.w, m.x, m.y, m.z);
}
template <class T> inline T vec2wxyz(Eigen::Vector4d v) {
  return make_xyzw<T>(v(1), v(2), v(3), v(0));
}
inline tf::Quaternion vec2quat(Eigen::Vector4d v) {
  return tf::Quaternion(v[0], v[1], v[2], v[3]);
}
template <class T> inline T quat2xyzw(Eigen::Quaterniond q) {
