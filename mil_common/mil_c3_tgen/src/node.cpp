...
subjugator::C3Trajectory::Point Point_from_PoseTwist(const Pose &pose,
  ...
  res.qdot.tail(3) = (Eigen::Matrix3d() << 1, sin(res.q[3]) * tan(res.q[4]),
                      cos(res.q[3]) * tan(res.q[4]), 0, cos(res.q[3]),
                      -sin(res.q[3]), 0, sin(res.q[3]) / cos(res.q[4]),
                      cos(res.q[3]) / cos(res.q[4])).finished() *
...
PoseTwist PoseTwist_from_PointWithAcceleration(
  ...
  Eigen::Matrix3d worldangvel_from_eulerrates =
      (Eigen::Matrix3d() << 1, 0, -sin(p.q[4]), 0, cos(p.q[3]),
       sin(p.q[3]) * cos(p.q[4]), 0, -sin(p.q[3]),
       cos(p.q[3]) * cos(p.q[4])).finished();

  res.twist.linear = vec2xyz<Vector3>(tf::Matrix3x3(orient.inverse()) *
                                      vec2vec(p.qdot.head(3)));
  res.twist.angular =
      vec2xyz<Vector3>(worldangvel_from_eulerrates * p.qdot.tail(3));

  res.acceleration.linear = vec2xyz<Vector3>(tf::Matrix3x3(orient.inverse()) *
                                             vec2vec(p.qdotdot.head(3)));
  res.acceleration.angular =
      vec2xyz<Vector3>(worldangvel_from_eulerrates * p.qdotdot.tail(3));
...
Pose Pose_from_Waypoint(const subjugator::C3Trajectory::Waypoint &wp) {
  ...
  quaternionTFToMsg(
      tf::createQuaternionFromRPY(wp.r.q[3], wp.r.q[4], wp.r.q[5]),
      res.orientation);
...
struct Node {
  ...
  actionlib::SimpleActionServer<sub8_msgs::MoveToAction> actionserver;
  ...
  bool set_disabled(SetDisabledRequest &request,
                    SetDisabledResponse &response) {
  ...
  Node() : private_nh("~"), actionserver(nh, "moveto", false), disabled(false) {

    fixed_frame = getParam<std::string>(private_nh, "fixed_frame");
    body_frame = getParam<std::string>(private_nh, "body_frame");

    limits.vmin_b = getParam<subjugator::Vector6d>(private_nh, "vmin_b");
    limits.vmax_b = getParam<subjugator::Vector6d>(private_nh, "vmax_b");
    limits.amin_b = getParam<subjugator::Vector6d>(private_nh, "amin_b");
    limits.amax_b = getParam<subjugator::Vector6d>(private_nh, "amax_b");
    limits.arevoffset_b = getParam<Eigen::Vector3d>(private_nh, "arevoffset_b");
    limits.umax_b = getParam<subjugator::Vector6d>(private_nh, "umax_b");
    traj_dt =
        getParam<ros::Duration>(private_nh, "traj_dt", ros::Duration(0.0001));

    odom_sub = nh.subscribe<Odometry>(
        "odom", 1, boost::bind(&Node::odom_callback, this, _1));

    trajectory_pub = nh.advertise<PoseTwistStamped>("trajectory", 1);
    waypoint_pose_pub = private_nh.advertise<PoseStamped>("waypoint", 1);

    update_timer = nh.createTimer(ros::Duration(1. / 50),
                                  boost::bind(&Node::timer_callback, this, _1));

    actionserver.start();

    set_disabled_service =
        private_nh.advertiseService<SetDisabledRequest, SetDisabledResponse>(
  ...
  void timer_callback(const ros::TimerEvent &) {
    ...
    if (actionserver.isNewGoalAvailable()) {
      boost::shared_ptr<const sub8_msgs::MoveToGoal> goal =
    ...
    if (actionserver.isPreemptRequested()) {
      ...
      // don't try to make output c3 continuous when cancelled - instead stop as
      // quickly as possible
      c3trajectory.reset(
          new subjugator::C3Trajectory(current_waypoint.r, limits));
      c3trajectory_t = now;
    }
    ...
    msg.posetwist =
        PoseTwist_from_PointWithAcceleration(c3trajectory->getCurrentPoint());
    ...
    if (actionserver.isActive() &&
        c3trajectory->getCurrentPoint().is_approximately(
            current_waypoint.r, max(1e-3, linear_tolerance),
...
int main(int argc, char **argv) {
  ros::init(argc, argv, "navigator_c3_trajectory_generator");
